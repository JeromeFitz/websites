...
const nonprofits = [
  ...
  "Planned Parenthood, Shepherd’s Heart Veterans Home",
  "SisTers PGH",
...
function Comedy() {
  return (
    <>
      ...
      <Text>
        For the past <Code variant="ghost">10+</Code> years I’ve been directing, producing, writing,
        and performing sketch comedy, musicals, and improv. Feel free to check some out in the{" "}
        <Link asChild>
      ...
      <br />
      <Text>
        Based in <Strong>NYC</Strong> (<Text className="line-through">Pittsburgh, PA</Text>) I’ve
        had shows in <Em>Chicago</Em>, <Em>Cleveland</Em>, <Em>Detroit</Em>, <Em>New York City</Em>,{" "}
        <Em>Philadelphia</Em>, <Em>Pittsburgh</Em>, <Em>San Diego</Em>, <Em>San Francisco</Em>, and
        more.
      </Text>
      <br />
      <br />
      <Text>
        ...
        <Em>Sucrets</Em>. On the jumbotron before games for the <Em>Pittsburgh Pirates</Em> at PNC
        Park. Or featured in{` `}
        <Em>The Onion</Em>. (It’s okay if not though.)
      </Text>
      <Separator my="4" orientation="horizontal" size="4" />
      <Text size="1">
        I’ve also been extremely fortunate to take courses from some of the funniest people in the
        comedy world and as a result – <Strong>thee world</Strong>:
        <br />
        {teachers.map((teacher, i) => {
          return (
            <Fragment key={`teacher-${i}`}>
              <Em>{teacher.title}</Em>,{` `}
            </Fragment>
          );
        })}
...
function Engineering() {
  return (
    <>
      <Text className="hidden">
        <Em>“Work hard, be kind, and amazing things will happen.” – Conan O’Brien</Em>
        <br />
        <br />
      </Text>
      <Text>
        I navigate complex technical challenges that align with organizational goals through
        workflow optimization. I do this by focusing on a collaborative culture of shared
        understanding that creates and maintains high-performing departments that highlight
        continuous improvement and engineering excellence.
        <br />
        <br />
        My focus is on consistently shipping products that are secure and reliable at scale. My
        drive as an engineering leader is to equally scale the impact of those I've been entrusted
        to lead. I encourage and grow next generation leaders and seek to recognize, foster, and
        retain talent.
        <br />
        <br />
        <Em>
      ...
      <Text>
        ...
        (<Em>rip</Em>), and am currently doing international business things at a company made
        famous by Paul Rand (and for their <Em>international business machinery</Em>).
        <br />
        <br />
        I’ve also worked with Fortune 100, start-ups, and everywhere in-between. Leading initiatives
        and teams through{" "}
        <Link asChild>
        ...
        so all facets of the business are clear on what defines done and how together we can share
        responsibility, success, and grow in a positive manner as a collective and individuals.
      </Text>
      <Separator my="4" orientation="horizontal" size="4" />
      <Text size="1">
...
function NonProfit() {
  return (
    <>
      ...
      <Text>
        Outside of ‘traditional work’ I have been fortunate enough to volunteer in the nonprofit
        sector:
      </Text>
      <br />
      <br />
      <UL>
        <LI>
          <Text>Arcade Comedy Theater 501(c)(3), founding Leaderboard Member (2014–Present)</Text>
        </LI>
        <LI>
          <Text>
        ...
        <LI>
          <Text>American Heart Association 501(c)(3), Worldwide Charter Member (2006–2012)</Text>
        </LI>
      </UL>
      <Separator my="4" orientation="horizontal" size="4" />
      <Text size="1">
        As well as organizing and/ or performing in benefits for:{` `}
        <Em>{nonprofits.join(", ")}, & more…</Em>
        {/* {nonprofits.map((nonprofit, i) => (
          <Em className="break-keep" key={`np-${i}`}>
            {nonprofit},{` `}
          </Em>
        ))} */}
      </Text>
    ...
    <>
      {data.map((item: any, i: number) => {
        ...
        return (
          <Flex
            asChild
            className="scroll-mt-25 border-none"
            direction="column"
            gap="12"
            key={`section--${item.id}`}
            width="100%"
          >
            <section id={`${item.id}`}>
              ...
              >
                <Flex
                  className="rounded-b-3 bg-accent-9 aspect-square items-center overflow-visible"
                  gap="0"
              ...
              <Box width="100%">
                <Box className="max-w-screen-sm">{item.content ? item.content : <></>}</Box>
              </Box>
            </section>
          </Flex>
        );
...
function SectionLegend({ data }: { data: any }) {
  return (
    <Box className="rounded-3 border-gray-7 border-1">
      {data.map((item: any, i: number) => {
        const Icon = item.icon;
        return (
          <Flex
            asChild
            className={cx(
              "group hover:bg-accent-4 items-center overflow-visible no-underline",
