...
import { useThemeToggle } from "@jeromefitz/shared/hooks/use-theme-toggle";
...
function ThemeToggle() {
  ...
  return (
    <>
      <button className="hover:text-accent-11 flex" onClick={() => handleThemeToggle()}>
        <span className="flex flex-row-reverse items-center justify-end gap-1 align-middle">
          <span className="">Toggle Theme</span>
          <span className="mr-2 size-4">{loaded && <Icon className="text-inherit" />}</span>
        </span>
      </button>
    </>
  );
