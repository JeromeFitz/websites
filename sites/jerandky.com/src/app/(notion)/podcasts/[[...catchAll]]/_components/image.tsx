...
// import { isImageExpired } from 'next-notion/src/utils/get-aws-image'
...
async function Image({ properties }) {
   ...
   * Image Information
   */
  const imageSeoDescription = getPropertyTypeDataEpisode(properties, "SEO.Image.Description");
  ...
  if (!isCached && !!imageUrl) {
    const { getImage } = await import("@jeromefitz/shared/plaiceholder/get-image");
