...
function getFormulaData({ data, type }: { data: any; type: string }) {
  ...
  console.dir(`[info] getFormulaData return is not a number|string: ${dataReturn.type}`);
  return dataReturn[dataReturn.type];
}

function getMultiSelectData({ data, type }: { data: MultiSelectData; type: string }) {
  // properties['Tags']
  const typeData = data[type];
...
function getRollupData({ data, type }: { data: RollupData; type: string }) {
  ...
  console.dir(`[info] getRollupData return is not a array|multi_select: ${dataReturn.type}`);
  dataReturn = dataReturn[dataReturn.type];
